...
(defn drop-by-position [coll pos]
  (concat (take pos coll) (drop (inc pos) coll)))

(defn is-level-could-be-safe [nums]
    (some is-level-safe (map #(drop-by-position nums %) (range (count nums)))))

(defn score-level [nums]
  (if (is-level-could-be-safe nums)
    1
    0))
